...
class MaterialWidgetFactory extends BaseWidgetFactory {
  ...
  Widget createButton({required BuildContext context, required Widget content, Widget? icon, Key? key, ButtonStyle? style, bool showLoadingIndicator = true, bool isLoading = false, Function? onPressed}) {
    ...
    if (icon != null) {
      return FilledButton.icon(
        key: key,
        icon: (showLoadingIndicator && isLoading) ? loadingIndicator : icon,
        label: content,
    ...
    return FilledButton(
      ...
      child: Row(
        ...
        children: [
          if (showLoadingIndicator && isLoading) ...[loadingIndicator, const SizedBox(width: 8)],
          content,
        ],
      ),
    );
  ...
  Widget createLoadingIndicator(BuildContext context, {Color? color, double? width = 20, double height = 20}) {
    return createCard(
      ...
      border: Border.all(color: color ?? Theme.of(context).colorScheme.primary, width: 1),
