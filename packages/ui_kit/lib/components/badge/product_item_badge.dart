...
class ProductBadgeInfo {
  ...
  final IconData icon;
  final Color color;
  const ProductBadgeInfo({required this.name, required this.icon, required this.color});
  ...
  static List<ProductBadgeInfo> getProductBadgeInfo(Product product) {
    ...
    if (product.isMembershipProduct) {
      badgeInfos.add(const ProductBadgeInfo(name: 'Membership', icon: Icons.card_membership, color: Colors.grey));
    }
    return badgeInfos;
  }
}
